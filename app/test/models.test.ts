...
describe("Event model", () => {
	...
	it("requires title, detail, location, date and time", async () => {
		await expectValidationError(
			Event.create({ ...baseEvent, title: undefined }),
		);
		await expectValidationError(
			Event.create({ ...baseEvent, date: undefined }),
		);
	});
...
describe("Payment model", () => {
	...
	it("prevents duplicate membership dues per user per month", async () => {
		...
		await expectDuplicateKey(Payment.create({ ...dues, reference: "dues-2" }));
	});
...
describe("InviteCode model", () => {
	...
	it("enforces unique email and unique invite code", async () => {
		...
		await expectDuplicateKey(
			InviteCode.create({ ...base, email: "a@b.com", inviteCode: "CODEB" }),
		);
		await expectDuplicateKey(
			InviteCode.create({ ...base, email: "c@d.com", inviteCode: "CODEA" }),
		);
	});
});
