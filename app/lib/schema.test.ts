...
const safeParse = <T>(schema: ZodType<T>, data: unknown) =>
...
describe("signInSchema / forgotPasswordSchema", () => {
	...
	it("rejects a bad email", () => {
		expect(
			safeParse(signInSchema, { email: "nope", password: validPassword })
				.success,
		).toBe(false);
	});
	...
	it("forgotPasswordSchema requires a valid email", () => {
		expect(
			safeParse(forgotPasswordSchema, { email: "ada@example.com" }).success,
		).toBe(true);
		expect(safeParse(forgotPasswordSchema, { email: "nope" }).success).toBe(
			false,
		);
	});
});
...
describe("resetPasswordSchema", () => {
	it("accepts a strong new password", () => {
		expect(
			safeParse(resetPasswordSchema, { newPassword: validPassword }).success,
		).toBe(true);
	});
...
describe("media schemas", () => {
	it("UploadSignatureSchema requires a folder of at least 2 chars", () => {
		expect(safeParse(UploadSignatureSchema, { folder: "photos" }).success).toBe(
			true,
		);
		expect(safeParse(UploadSignatureSchema, { folder: "p" }).success).toBe(
			false,
		);
	});
	...
	it("uploadSchema requires files and folder", () => {
		expect(
			safeParse(uploadSchema, { files: ["a", "b"], folder: "x" }).success,
		).toBe(true);
		expect(safeParse(uploadSchema, { files: [], folder: "x" }).success).toBe(
			false,
		);
	...
	it("deleteMediaSchema requires at least one public id", () => {
		expect(safeParse(deleteMediaSchema, { publicIds: ["id1"] }).success).toBe(
			true,
		);
	...
	it("updateUserAvatarSchema allows partial avatar updates", () => {
		...
		expect(
			safeParse(updateUserAvatarSchema, { image: "url", imagePublicId: "pid" })
				.success,
		).toBe(true);
	});
});
...
describe("updateProfileSchema", () => {
	...
	it("accepts a valid phone with a leading +", () => {
		expect(
			safeParse(updateProfileSchema, { phone: "+2348012345678" }).success,
		).toBe(true);
	...
	it("rejects a phone that is too short", () => {
		expect(safeParse(updateProfileSchema, { phone: "+234" }).success).toBe(
			false,
		);
	});

	it("accepts a valid gender and rejects unknown genders", () => {
		expect(safeParse(updateProfileSchema, { gender: "male" }).success).toBe(
			true,
		);
		expect(safeParse(updateProfileSchema, { gender: "robot" }).success).toBe(
			false,
		);
	});
...
describe("bank account schemas", () => {
	it("createBankAccountSchema accepts valid account info", () => {
		expect(
			safeParse(createBankAccountSchema, {
	...
	it("resolveBankAccountSchema requires a 10-digit account number", () => {
		expect(
			safeParse(resolveBankAccountSchema, {
...
describe("event schemas", () => {
	...
	it("createEventSchema rejects short title and invalid event type", () => {
		const badTitle = safeParse(createEventSchema, {
			...validEvent,
			title: "AB",
		});
		expect(badTitle.success).toBe(false);
		const badType = safeParse(createEventSchema, {
			...validEvent,
			eventType: "hangout",
		});
	...
	it("updateEventSchema validates date/time are parseable", () => {
		...
		const res = safeParse(updateEventSchema, {
			...validEvent,
			date: "garbage",
		});
		expect(res.success).toBe(false);
...
describe("initializePaymentSchema", () => {
	it("accepts a membership dues payment", () => {
		expect(
			safeParse(initializePaymentSchema, {
				amount: 5000,
...
describe("payment/subscription verification", () => {
	it("verifyPaymentSchema requires a reference", () => {
		expect(safeParse(verifyPaymentSchema, { reference: "ref-1" }).success).toBe(
			true,
		);
	...
	it("cancelSubscriptionSchema requires a reference or a code+token pair", () => {
		expect(
			safeParse(cancelSubscriptionSchema, { reference: "r" }).success,
		).toBe(true);
		expect(
			safeParse(cancelSubscriptionSchema, { code: "c", token: "t" }).success,
		).toBe(true);
		expect(safeParse(cancelSubscriptionSchema, {}).success).toBe(false);
		expect(safeParse(cancelSubscriptionSchema, { code: "c" }).success).toBe(
			false,
		);
	});
});
...
describe("changePasswordSchema", () => {
	it("accepts matching strong passwords", () => {
		expect(
			safeParse(changePasswordSchema, {
...
describe("ChangeEmailSchema", () => {
	it("accepts a valid email and rejects invalid ones", () => {
		expect(
			safeParse(ChangeEmailSchema, { newEmail: "ada@example.com" }).success,
		).toBe(true);
		expect(
			safeParse(ChangeEmailSchema, { newEmail: "not-an-email" }).success,
		).toBe(false);
	});
});
...
describe("transfer schemas", () => {
	it("createTransferSchema accepts a valid transfer", () => {
		expect(
			safeParse(createTransferSchema, {
				userId: "u-1",
				amount: 5000,
			}).success,
		).toBe(true);
	...
	it("verifyTransferSchema and retryTransferSchema require a reference", () => {
		expect(safeParse(verifyTransferSchema, { reference: "r" }).success).toBe(
			true,
		);
		expect(safeParse(verifyTransferSchema, {}).success).toBe(false);
		expect(safeParse(retryTransferSchema, { reference: "r" }).success).toBe(
			true,
		);
	...
	it("finalizeTransferSchema requires a 6-digit OTP", () => {
		expect(
			safeParse(finalizeTransferSchema, { transferCode: "t", otp: "123456" })
				.success,
		).toBe(true);
		expect(
			safeParse(finalizeTransferSchema, { transferCode: "t", otp: "12345" })
				.success,
		).toBe(false);
		expect(
			safeParse(finalizeTransferSchema, { transferCode: "t", otp: "1234567" })
				.success,
		).toBe(false);
		expect(safeParse(finalizeTransferSchema, { otp: "123456" }).success).toBe(
			false,
		);
	});
});
...
describe("sendBirthdayReminderSchema", () => {
	it("requires a userId", () => {
		expect(
			safeParse(sendBirthdayReminderSchema, { userId: "u-1" }).success,
		).toBe(true);
