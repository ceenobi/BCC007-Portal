...
describe("helpdeskKnowledgeBase integrity", () => {
	...
	it("every article has required fields populated", () => {
		for (const article of helpdeskKnowledgeBase) {
			...
			expect(
				article.category.trim().length,
				`${article.id} category`,
			).toBeGreaterThan(0);
			expect(article.icon.trim().length, `${article.id} icon`).toBeGreaterThan(
				0,
			);
			expect(
				article.content.trim().length,
				`${article.id} content`,
			).toBeGreaterThan(0);
			expect(Array.isArray(article.keywords)).toBe(true);
			expect(article.keywords.length, `${article.id} keywords`).toBeGreaterThan(
				0,
			);
		}
	});
