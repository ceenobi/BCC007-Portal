...
function PriorityBadge({ priority }: { priority: TicketData["priority"] }) {
	const config = ticketPriorityConfig[priority] ?? ticketPriorityConfig.medium;
	return (
		<Badge className={cn("shrink-0 gap-1.5", config.className)}>
			<span
				className={cn("size-1.5 shrink-0 rounded-full", config.dotClassName)}
...
function CategoryBadge({ category }: { category: TicketData["category"] }) {
	const categoryClass = {
		account: "border-transparent bg-sky-500/10 text-sky-600 dark:text-sky-400",
		payment:
		...
		other: "border-transparent bg-zinc-500/10 text-zinc-600 dark:text-zinc-400",
	}[category];
	return <Badge className={cn("capitalize", categoryClass)}>{category}</Badge>;
}
...
export default function TicketList({ tickets, user, admins }: TicketListProps) {
	...
	const columns = useMemo<ColumnDef<TicketData>[]>(
				...
				? [
						{
							...
							cell: ({ row }: { row: { original: TicketData } }) => (
								<div className="flex items-center justify-end">
									<AssignTicketModal ticket={row.original} admins={admins} />
								</div>
							),
		...
		[canManageStatus, canAssign, admins, handleStatusChange],
	);
