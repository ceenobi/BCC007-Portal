...
export default function Filter() {
  ...
  const activeStatus = paymentStatus.find(
    (item) => item.value === filters.paymentStatus,
  );
      ...
      >
          ...
          <DropdownMenuContent align="start" className="w-44">
            <DropdownMenuGroup>
              {paymentStatus.map((item) => (
                <DropdownMenuCheckboxItem
                  key={item.value}
                  checked={filters.paymentStatus === item.value}
        ...
        >
          <SelectTrigger className="w-fit text-xs border focus:outline-lightBlue focus:ring-lightBlue">
            <SelectValue placeholder="All payment types" className="font-medium" />
          </SelectTrigger>
          <SelectContent>
