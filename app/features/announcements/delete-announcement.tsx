...
export default function DeleteAnnouncement({
  ...
  useEffect(() => {
    if (actionData?.success) {
      toast.success(actionData.message || "Announcement deleted successfully");
  ...
  return (
    <>
      ...
      <Modal
        ...
        description={`Permanently delete "${announcement.title}"? This cannot be undone.`}
      >
        <Separator />
        <div className="space-y-4 p-2">
          <div className="flex items-start gap-2 rounded-md border border-destructive/40 bg-destructive/10 px-3 py-2 text-sm text-destructive">
            ...
            <span>
              The announcement and its featured image (if any) will be
