...
export default function TransferMonthlyFlowsChart({
  data,
  summary,
}: {
  data: TransferReportData["monthlyBreakdown"];
  summary: TransferReportData["monthlySummary"];
}) {
  return (
    <ChartCard
      title={`Transfer Flows (${summary.year})`}
      description={`${formatMoney(summary.total)} sent across ${data.length} month${data.length === 1 ? "" : "s"}`}
    >
      {data.every((m) => m.total === 0 && m.count === 0) ? (
        <div className="flex h-64 items-center justify-center rounded-md border border-dashed text-sm text-muted-foreground">
          No transfer flows for this period.
        </div>
      ) : (
