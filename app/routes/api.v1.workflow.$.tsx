...
export const action = async (args: any) => {
	...
	const { runStatusUpdatesWorkflow } = await import(
		"~/.server/workflows/status.workflow"
	);
	const { runBirthdayRemindersWorkflow } = await import(
		"~/.server/workflows/birthday.workflow"
	);
	const { announcementCreatedWorkflow } = await import(
		"~/.server/workflows/announcement.workflow"
	);
	const { runDashboardRefreshWorkflow } = await import(
		"~/.server/workflows/dashboard.workflow"
	);
	const { sendPaymentConfirmationWorkflow } = await import(
		"~/.server/workflows/payment.workflow"
	);
	const { runSubscriptionSyncWorkflow } = await import(
		"~/.server/workflows/subscription-sync.workflow"
	);
