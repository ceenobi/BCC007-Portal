...
const CookieCategories: Record<CookieCategory, string> = {
  essential: "Essential (Better Auth session – required for login and account access)",
...
export function CookieBanner() {
  const [consentedCategories, setConsentedCategories] = useState<CookieCategory[]>([]);
  ...
  const essentialPart = hasEssential
    ...
    : "Essential cookies are always active to keep you signed in and secure.";
  const preferencesPart = hasPreferences && consentedCategories.includes("preferences")
  ...
  return showBanner ? (
    <div
      className="fixed top-20 left-0 right-0 z-50 bg-white border-b border-border p-4 sm:p-6 shadow-lg outline outline-2 outline-offset-2 outline-indigo-600"
      aria-live="polite"
      aria-atomic="true"
    >
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-sm text-muted-foreground mb-3">
          <span className="font-medium text-mainBlue">
            {essentialPart}
          </span> This website uses cookies to enhance your experience.
        </div>
        ...
        <div className="mt-4 space-y-2">
            ...
            <input
              ...
              onChange={(e) =>
                setConsentedCategories(
                  e.target.checked ? ["essential", "preferences"] : ["essential"]
                )
              }
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              ...
              onChange={(e) =>
                setConsentedCategories(
                  e.target.checked ? ["essential", "advertising"] : ["essential"]
                )
              }
              className="rounded cursor-pointer"
            />
        ...
        <div className="mt-4 flex gap-3">
          ...
          <button
            onClick={() => handleAccept(["essential", "preferences", "analytics", "advertising"])}
