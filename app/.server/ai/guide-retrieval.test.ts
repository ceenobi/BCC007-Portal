...
import {
	...
	searchGuide,
	searchGuideVector,
} from "~/.server/ai/guide-retrieval";
	...
	it("returns keyword results when Vector query returns null", async () => {
		vi.doMock("~/.server/ai/vector", async () => {
			const actual = await vi.importActual<
				typeof import("~/.server/ai/vector")
			>("~/.server/ai/vector");
			return {
				...actual,
				isVectorEnabled: () => true,
				queryGuideVector: async () => null,
			};
		});
		const { searchGuideVector: sv } = await import(
			"~/.server/ai/guide-retrieval"
		);
