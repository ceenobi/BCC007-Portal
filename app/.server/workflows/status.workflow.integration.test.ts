...
describe("runStatusUpdatesWorkflow (integration)", () => {
	...
	it("transitions events to ongoing/completed and notifies recipients", async () => {
		const eFuture = await makeEvent(
			"Future",
			"upcoming",
			addDays(startOfToday(), 1),
		);
		const eOngoingNow = await makeEvent(
			"Ongoing Now",
			"upcoming",
			startOfToday(),
		);
		const ePastUpcoming = await makeEvent(
			"Past Upcoming",
			"upcoming",
			addDays(startOfToday(), -1),
		);
		const ePastOngoing = await makeEvent(
			"Past Ongoing",
			"ongoing",
			addDays(startOfToday(), -1),
		);
		const eTodayOngoing = await makeEvent(
			"Today Ongoing",
			"ongoing",
			startOfToday(),
		);
		...
		const statusOf = async (id: unknown) =>
			(await Event.findById(id).lean())?.status;
		...
		expect(await Notification.countDocuments({ type: "event_ongoing" })).toBe(
			2,
		);
		expect(await Notification.countDocuments({ type: "event_completed" })).toBe(
			4,
		);
	});
