...
export class PaystackService {
	...
	static async verifyTransfer(
		...
		try {
			const { data } = await getPaystack().get(`/transfer/verify/${reference}`);
	...
	static async initializePayment(
		...
		try {
			...
			if (membershipLockId) {
				await Payment.deleteOne({ _id: membershipLockId }).catch(
					() => undefined,
				);
			}
			logger.error(
	...
	static async verifyPayment(
		...
		// confirmation run.
		const claimed = await PaystackService._claimCompletedPayment(
			tx.reference,
			paymentUpdate,
		);
		...
		await PaystackService._triggerPaymentConfirmation(
			metadata.userId,
			tx.amount / 100,
			tx.reference,
		);
		...
		try {
			switch (eventType) {
				...
				case "transfer.success": {
					...
					if (transfer) {
						await invalidateCache(
							`transfers:user:${transfer.userId.toString()}:*`,
						);
					}
					await PaystackService._triggerTransferNotification(
						data.reference,
						"success",
					);
					break;
					...
					const transfer = await Transfer.findOneAndUpdate(
						{ reference: data.reference },
						{
							$set: {
								transferCode: data.transfer_code,
								status: "reversed",
								failureReason: data.failure_reason || "Transfer reversed",
							},
						},
						{ returnDocument: "after" },
					);
