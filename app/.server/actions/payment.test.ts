...
const makePayment = (
	...
	Payment.create({
		...
		reference:
			opts.reference ?? `BCC-PAY-${Math.random().toString(36).slice(2, 8)}`,
...
describe("initializePayment", () => {
	...
	it("delegates to Paystack and records an audit log on success", async () => {
		...
		expect(initPaymentMock).toHaveBeenCalledWith(
			validPayload,
			session("member", userId).user,
		);
		expect(await AuditLog.countDocuments({ action: "PAYMENT_INITIATED" })).toBe(
			1,
		);
	});
});
...
describe("verifyPayment", () => {
	...
	it("returns 401 without a session", async () => {
		getSessionMock.mockResolvedValue(null as never);
		const res = await verifyPayment(request(), {
	...
	it("delegates to Paystack and records an audit log on success", async () => {
		...
		expect(await AuditLog.countDocuments({ action: "PAYMENT_SUCCESS" })).toBe(
			1,
		);
	});
});
...
describe("cancelSubscription", () => {
	...
	it("returns 401 without a session", async () => {
		getSessionMock.mockResolvedValue(null as never);
		const res = await cancelSubscription(request(), {
			code: "SUB_x",
			token: "tok",
		});
		expect(res.status).toBe(401);
	...
	it("delegates to Paystack and records an audit log on success", async () => {
		...
		expect(
			await AuditLog.countDocuments({ action: "SUBSCRIPTION_CHANGE" }),
		).toBe(1);
	});
});
...
describe("getGroupPayments", () => {
	...
	it("returns 401 without a session", async () => {
		getSessionMock.mockResolvedValue(null as never);
		const res = await getGroupPayments({
			request: request(),
			page: 1,
			limit: 10,
			query: undefined,
...
describe("getUserPaymentReports", () => {
	...
	it("returns 401 without a session", async () => {
		getSessionMock.mockResolvedValue(null as never);
		const res = await getUserPaymentReports({
			request: request(),
			period: undefined,
			paymentStatus: undefined,
			paymentType: undefined,
...
describe("getGroupPaymentReports", () => {
	...
	it("returns 401 without a session", async () => {
		getSessionMock.mockResolvedValue(null as never);
		const res = await getGroupPaymentReports({
			request: request(),
			period: undefined,
			paymentStatus: undefined,
			paymentType: undefined,
	...
	it("flags the group on track when every onboarded member has paid this month", async () => {
		await makePayment(memberId, {
			paymentStatus: "completed",
			amount: 2000,
			reference: "BCC-PAY-D1",
		});
		...
		await makePayment(adminId, {
			paymentStatus: "completed",
			amount: 2000,
			reference: "BCC-PAY-D3",
		});
