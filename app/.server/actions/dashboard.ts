...
import { getGroupPaymentReports, getUserPaymentReports } from "./payment";
		...
		const [
			...
			fetchWithCache("dashboard:latest-announcement", 300, async () =>
				Announcement.findOne({ status: "published" })
					...
					.select(
						"title content status isPinned featuredImage featuredImageId publishedAt createdAt",
					)
					.lean(),
			),
		]);
...
export async function sendBirthdayReminder(request: Request, payload: unknown) {
	return tryCatchWrapper(async () => {
