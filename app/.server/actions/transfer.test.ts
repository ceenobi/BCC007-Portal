...
describe("initiateTransfer", () => {
	...
	it("creates a recipient, transfer row and audit log on success", async () => {
		...
		expect(
			await AuditLog.countDocuments({ action: "TRANSFER_INITIATED" }),
		).toBe(1);
	});
...
describe("verifyTransfer", () => {
	...
	it("returns 404 for an unknown reference", async () => {
		const res = await verifyTransfer(request(), {
			reference: "BCC-TRF-MISSING",
		});
		expect(res.status).toBe(404);
	});
...
describe("finalizeTransfer", () => {
	...
	it("finalizes an otp transfer and releases the claim", async () => {
		...
		expect(
			await AuditLog.countDocuments({ action: "TRANSFER_FINALIZED" }),
		).toBe(1);
	});
...
describe("retryTransfer", () => {
	...
	beforeEach(async () => {
		...
		verifyMock.mockResolvedValue({
			...
			data: {
				status: "failed",
				transfer_code: "TRF_fail1",
				failure_reason: "NUBAN invalid",
			},
		} as never);
		initiateMock.mockResolvedValue({
			status: true,
	...
	it("returns 404 for an unknown reference", async () => {
		const res = await retryTransfer(request(), {
			reference: "BCC-TRF-MISSING",
		});
		expect(res.status).toBe(404);
	});
	...
	it("creates a new transfer row and links it to the original", async () => {
		...
		expect(await AuditLog.countDocuments({ action: "TRANSFER_RETRIED" })).toBe(
			1,
		);
	});
});
