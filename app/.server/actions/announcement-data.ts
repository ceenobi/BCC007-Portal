	...
	return tryCatchWrapper(async () => {
		...
		if (!hasPermission(session.user.role, "MANAGE_ANNOUNCEMENTS")) {
			...
			return Response.json(
				{
					success: false,
					message: "Access denied. Requires 'MANAGE_ANNOUNCEMENTS' permission.",
				},
				{ status: 403 },
			);
		}
		...
		const announcement = await Announcement.create({
			...
			if ((err as { code: number; message: string })?.code === 11000 && idempotencyKey) {
		...
		const body = await fetchWithCache(cacheKey, 300, async () => {
			...
			if (query) {
				...
				if (mongoose.Types.ObjectId.isValid(query)) {
					;(matchStage.$or as unknown[]).push({ _id: query });
				}
			}
...
async function triggerAnnouncementBroadcast(announcement: {
	...
	await workflowClient
		...
		.catch((err: unknown) =>
			logger.error("Failed to trigger announcement-created workflow:"),
		);
}
