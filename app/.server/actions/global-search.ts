...
> = {
	member: {
		label: "Members",
		viewAllHref: (query) =>
			`/dashboard/members?query=${encodeURIComponent(query)}`,
	},
	event: {
		label: "Events",
		viewAllHref: (query) =>
			`/dashboard/events?query=${encodeURIComponent(query)}`,
	},
	payment: {
		label: "Payments",
		viewAllHref: (query) =>
			`/dashboard/payments?query=${encodeURIComponent(query)}`,
	},
	transfer: {
		label: "Transfers",
		viewAllHref: (query) =>
			`/dashboard/transfers?query=${encodeURIComponent(query)}`,
	},
	ticket: {
		label: "Tickets",
		viewAllHref: (query) =>
			`/dashboard/help-center?query=${encodeURIComponent(query)}`,
	},
	audit: {
		label: "Activity",
...
async function buildSections({
	...
	return {
		...
		sections: sections.filter(
			(section): section is GlobalSearchSection => section !== null,
		),
	};
}
...
async function buildPaymentsSection({
	...
	const matchStage: Record<string, any> = canViewGroup ? {} : { userId };
	matchStage.$or = [
		{ reference: regex },
		{ note: regex },
