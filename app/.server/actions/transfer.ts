...
export async function getAvailableBalance(request: Request) {
	return tryCatchWrapper(async () => {
		...
		const entry = await fetchWithCache("balance:available", 60, () =>
				...
				return Response.json(
					{
						success: false,
						message:
							finalized.message || "Failed to finalize transfer. Please retry.",
					},
					{ status: 400 },
				);
			}
...
export async function retryTransfer(
	...
	return tryCatchWrapper(async () => {
		...
		try {
			const remote = await PaystackService.verifyTransfer(
				result.data.reference,
			);
...
export async function getGroupTransferReports({
	...
	return tryCatchWrapper(async () => {
		...
		if (period && !(REPORT_PERIODS as readonly string[]).includes(period)) {
			return Response.json(
				{
					success: false,
		...
		const body = await fetchWithCache(cacheKey, 3600, async () => {
			...
			const aggregateResult = await runFacetAggregation(matchStage, dateFormat);
