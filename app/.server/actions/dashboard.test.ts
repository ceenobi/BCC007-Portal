...
const makePayment = (
	...
	Payment.create({
		...
		reference:
			opts.reference ??
			`BCC-PAY-DASH-${Math.random().toString(36).slice(2, 8)}`,
	});
...
const makeEvent = (
	userId: string,
	opts: { title?: string; status?: string; date?: Date } = {},
) =>
	Event.create({
...
describe("getDashboardData", () => {
	...
	it("returns personal-scoped sections for a member", async () => {
		...
		expect(body.body.upcomingBirthdays[0].name).toBe(
			`Member ${birthdayUserId.slice(0, 4)}`,
		);
	...
	it("includes balance, group reports and all audit logs for a super_admin", async () => {
		...
		expect(body.body.recentActivity).toHaveLength(2);
		expect(
			body.body.recentActivity
				.map((log: { action: string }) => log.action)
				.sort(),
		).toEqual(["MEMBER_VIEW", "SUPER_VIEW"]);
		expect(body.body.upcomingEvents).toHaveLength(1);
	});
});
