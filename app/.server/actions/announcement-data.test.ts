...
describe("getAnnouncements", () => {
	...
	it("rejects an invalid page or limit", async () => {
		...
		expect((await getAnnouncements(listParams({ limit: 101 }))).status).toBe(
			400,
		);
	});
	...
	it("filters by status", async () => {
		...
		await makeAnnouncement(adminId, {
			title: "Live Note",
			status: "published",
		});
...
describe("createAnnouncement", () => {
	...
	it("creates a draft announcement and records an audit log", async () => {
		...
		expect(
			await AuditLog.countDocuments({ action: "CREATE_ANNOUNCEMENT" }),
		).toBe(1);
	});
...
describe("updateAnnouncement", () => {
	...
	it("updates the announcement and records an audit log", async () => {
		...
		expect(
			await AuditLog.countDocuments({ action: "UPDATE_ANNOUNCEMENT" }),
		).toBe(1);
	});
...
describe("deleteAnnouncement", () => {
	...
	it("deletes the announcement, image and records an audit log", async () => {
		...
		expect(
			await AuditLog.countDocuments({ action: "DELETE_ANNOUNCEMENT" }),
		).toBe(1);
	});
});
