...
const makeEvent = (
	...
	Event.create({
		...
		...(opts.interestedMembers
			? { interestedMembers: opts.interestedMembers }
			: {}),
		...(opts.checkedInMembers
			? { checkedInMembers: opts.checkedInMembers }
			: {}),
...
describe("getEvents", () => {
	...
	it("filters by date range", async () => {
		await makeEvent(adminId, {
			title: "Mid Year",
			date: new Date("2030-06-01T10:00:00Z"),
		});
		await makeEvent(adminId, {
			title: "Next Year",
			date: new Date("2031-06-01T10:00:00Z"),
		});
...
describe("getEvent", () => {
	...
	it("returns 401 without a session", async () => {
		getSessionMock.mockResolvedValue(null as never);
		const res = await getEvent(request(), {
...
describe("cancelEvent", () => {
	...
	it("cancels an upcoming event, logs it and notifies interested members", async () => {
		...
		await vi.waitFor(async () => {
			expect(
				await Notification.countDocuments({ type: "event_cancelled" }),
			).toBe(2);
		});
	});
...
describe("toggleEventCheckIn", () => {
	...
	it("checks in an interested member and records an audit log", async () => {
		...
		expect(await AuditLog.countDocuments({ action: "EVENT_CHECK_IN" })).toBe(1);
	});
