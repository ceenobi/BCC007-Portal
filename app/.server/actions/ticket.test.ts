...
describe("createTicket", () => {
	...
	it("creates a ticket, audit log and notification for a valid payload", async () => {
		...
		await vi.waitFor(async () => {
			expect(
				await Notification.countDocuments({ type: "ticket_created" }),
			).toBe(1);
		});
		expect(triggerMock).toHaveBeenCalledTimes(1);
		expect(triggerMock).toHaveBeenCalledWith(
			expect.objectContaining({
				url: expect.stringContaining("ticket-confirmation"),
			}),
		);
	});
...
describe("ticketActions", () => {
	...
	const createTicketDoc = async (
		opts: { userId?: string; assignedTo?: string | null } = {},
	) =>
		Ticket.create({
	...
	it("returns 401 without a session", async () => {
		getSessionMock.mockResolvedValue(null as never);
		const res = await ticketActions(request(), {
			id: "some-id",
			status: "resolved",
		});
		expect(res.status).toBe(401);
	...
	it("returns 400 when no ticket id is provided", async () => {
		...
		const res = await ticketActions(request(), {
			status: "resolved",
		} as Parameters<typeof ticketActions>[1]);
		expect(res.status).toBe(401);
		const body = await res.json();
	...
	it("rejects a status that requires assignment while unassigning", async () => {
		getSessionMock.mockResolvedValue(
			session("super_admin", superAdminId) as never,
		);
		const ticket = await createTicketDoc();
		const res = await ticketActions(request(), {
			id: ticket._id.toString(),
	...
	it("assigns a ticket and notifies the assignee", async () => {
		...
		await vi.waitFor(async () => {
			expect(
				await Notification.countDocuments({ type: "ticket_assigned" }),
			).toBe(1);
		});
		expect(triggerMock).toHaveBeenCalledWith(
			expect.objectContaining({
				url: expect.stringContaining("ticket-assigned"),
			}),
		);
	});
	...
	it("resolves an assigned ticket and notifies the owner once", async () => {
		...
		const first = await ticketActions(request(), {
			id: ticket._id.toString(),
			status: "resolved",
		});
		expect(first.status).toBe(200);

		const second = await ticketActions(request(), {
			id: ticket._id.toString(),
			status: "resolved",
		});
		...
		await vi.waitFor(async () => {
			expect(
				await Notification.countDocuments({ type: "ticket_resolved" }),
			).toBe(1);
		});
	});
});
