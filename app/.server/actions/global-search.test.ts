...
describe("globalSearch", () => {
	...
	it("scopes payments, tickets and audit logs to the member and omits transfers", async () => {
		...
		const types = body.body.sections.map(
			(section: { type: string }) => section.type,
		);
		expect(types).toEqual(
			expect.arrayContaining(["member", "event", "payment", "ticket", "audit"]),
		);
	...
	it("returns all sections including transfers and audit logs for a super_admin", async () => {
		...
		expect(
			paymentSection.results.map((r: { title: string }) => r.title).sort(),
		).toEqual(["BCC-PAY-zebra-1", "BCC-PAY-zebra-2"]);
		...
		expect(
			auditSection.results.map((r: { title: string }) => r.title).sort(),
		).toEqual(["ZEBRA_LOGIN", "ZEBRA_SEARCH"]);
	});
});
