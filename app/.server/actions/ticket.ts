	...
	return tryCatchWrapper(async () => {
		...
		if (payload.status && !hasPermission(role, "MANAGE_TICKETS")) {
			...
			return Response.json(
				{
					success: false,
					message: "Unauthorized: You do not have permission to manage tickets",
				},
				{ status: 403 },
			);
		}
		if (
			...
			!hasPermission(role, "ASSIGN_TICKET")
		) {
		...
		if (!updatedTicket) {
			...
			const currentAssignedTo = ticket.assignedTo
				...
				: null;
			const statusOk = !payload.status || ticket.status === payload.status;
			const assignmentOk =
				payload.assignedTo === undefined ||
