...
async function getCheckRuns(owner, repo, sha) {
	...
	const response = await fetch(url, {
		headers: {
			...
			"User-Agent": "bcc007-portal",
		},
	});
	if (!response.ok) {
		throw new Error(
			`GitHub check-runs request failed: ${response.status} ${response.statusText}`,
		);
	}
	...
	try {
		...
		for (const name of REQUIRED_CHECKS) {
			...
			const failed = runs.some((run) =>
				FAILURE_CONCLUSIONS.includes(run.conclusion),
			);
			if (failed) {
				log(
					`required check "${name}" failed for ${VERCEL_GIT_COMMIT_SHA}: skipping build`,
				);
				process.exit(0);
			}
		}
