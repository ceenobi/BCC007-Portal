...
const destination = `${clientUrl}/api/v1/workflow/run-transfer-sync`;
const cron = "*/10 * * * *";
...
async function main() {
	...
	const existing = schedules.find((schedule) =>
		schedule.destination.includes("/run-transfer-sync"),
	);
...
main().catch((error) => {
	console.error("Failed to create/update transfer-sync schedule:", error);
	process.exit(1);
});
